...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Edit', href: `/admin/banks/${props.bank.id}/edit` },
];
...
const form = useForm<BankForm>({
  ...
  swift_code: props.bank.swift_code || '',
  description: props.bank.description || '',
  is_active: props.bank.is_active,
  admin_fee: props.bank.admin_fee,
  bank_type: props.bank.bank_type,
});

const submit = () => {
  form.put(`/admin/banks/${props.bank.id}`, {
    onSuccess: () => {
...
<template>
  <Head :title="`Edit ${bank.bank_name}`" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Edit {{ bank.bank_name }}</h1>
          <p class="text-muted-foreground">
            Perbarui informasi bank pembayaran
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Perbarui detail bank pembayaran
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="flex items-center gap-4 pt-4">
              ...
              <Button variant="outline" type="button" asChild>
                <Link :href="`/admin/banks/${bank.id}`">Batal</Link>
              </Button>
            </div>
          </form>
        </CardContent>
