...
watch(
    ...
    (plans) => {
        if (plans && plans.length > 0) {
            ...
            const basicPlan =
                plans.find((plan) => plan.plan_name.toLowerCase().includes('basic') || plan.plan_name.toLowerCase().includes('2gb')) || plans[0];
...
<template>
    ...
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative mx-4 max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <!-- Modal Header -->
            <div class="flex items-center justify-between border-b p-6">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600">Dapatkan penawaran terbaik dengan bundling hosting + domain</p>
                </div>
            ...
            <div class="p-6">
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <Card
                        v-if="hostingPlan"
                        class="relative border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 transition-all hover:shadow-lg"
                    >
                        <div class="absolute -top-3 left-1/2 -translate-x-1/2 transform">
                            <Badge class="bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-1 text-white">
                        ...
                        <CardContent class="space-y-4">
                            ...
                            <div v-if="selectedHostingPlan" class="rounded-lg border bg-white p-3">
                            ...
                            <!-- Bundle Price -->
                            <div class="rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 p-4 text-center text-white">
                                <div class="text-sm opacity-90">Total Harga Bundle</div>
                                <div class="text-xs line-through opacity-75">{{ formatPrice(bundlePrice) }}</div>
                                <div class="text-2xl font-bold">{{ formatPrice(finalBundlePrice) }}</div>
                                <div class="text-xs opacity-90">per tahun</div>
                            </div>

                            <Button
                                @click="handleSelectBundle"
                    ...
                    <!-- Domain Only Option -->
                    <Card class="border-2 border-gray-200 transition-all hover:shadow-lg">
                        ...
                        <CardContent class="space-y-4">
                            ...
                            <div class="rounded-lg bg-gray-50 p-4 text-center">
                                ...
                                <div class="mt-2 text-2xl font-bold text-gray-900">{{ formatPrice(domainPrice.selling_price) }}</div>
